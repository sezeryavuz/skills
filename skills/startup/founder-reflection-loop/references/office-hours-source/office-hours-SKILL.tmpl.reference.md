<!--
REFERENCE COPY - NOT AN ACTIVE SKILL

Vendored, read-only copy of the `office-hours` skill by Garry Tan (gstack),
source: https://github.com/garrytan/gstack (installed via
`npx skills add https://github.com/garrytan/gstack --skill office-hours`).

Kept so `founder-reflection-loop` can be audited against the skill it hands off to.
Root files are renamed from SKILL.md so they cannot be discovered as an active skill
definition. Do not edit; the installed copy is authoritative at runtime.

Note on .tmpl vs generated .md: the .tmpl files are the editable sources, but the
generated .md files expand placeholders and in places carry operative detail the
templates do not (e.g. the adversarial-review step and its 1-10 document score).
Read both. Frontmatter here uses gstack's schema (preamble-tier, version, triggers,
gbrain, allowed-tools) and is deliberately NOT carried into this skill.
-->
