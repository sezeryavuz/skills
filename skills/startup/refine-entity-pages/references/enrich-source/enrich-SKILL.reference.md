...
This is a vendored, read-only copy of the `enrich` skill by Garry Tan,
source: https://github.com/garrytan/gbrain (installed via
`npx skills add https://github.com/garrytan/gbrain --skill enrich`).

It is kept here so `refine-entity-pages` can be read and audited against the
protocol it wraps. It is renamed from SKILL.md so it cannot be discovered as an
active skill definition. Do not edit it, and do not treat it as authoritative
over the installed version - consult the installed skill at runtime.

Original frontmatter below uses another host's schema (version, triggers, tools,
mutating, writes_pages, writes_to). Those fields are intentionally NOT carried
into this skill's own frontmatter.
-->
